...
void Map::generate(uint16_t seed, WORLD_SIZE size) {
    ...
    AdditionalPuzzleLocations(win_rand() % (max4 - base4 + 1) + base4, tiles, &placed_puzzle_count);
    placeTransports(placed_transport_count);
    
    place_intermediate_world_thing();
}

#pragma mark -
int Map::DeterminePuzzleLocations(signed int iteration, int puzzle_count_to_place, uint16_t* world, int transport_count, int *placed_transport_count_ref, int blockade_count, int *placed_blockade_count_ref, int *placed_puzzle_count_ref)
{
    ...
    for(int i=0; i <= 144 && puzzle_count_to_place > 0; i++)
    {
        ...
        if (item_before != WORLD_ITEM_NONE && item_before != WORLD_ITEM_KEPT_FREE) {
            ...
            if ( item_before > WORLD_ITEM_TRAVEL_START && item_before != WORLD_ITEM_SPACEPORT && item_before != WORLD_ITEM_PUZZLE)
            {
                ...
                {
                    world[item_idx] = WORLD_ITEM_PUZZLE_CANDIDATE;
                    world[item_idx-10] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+10] = WORLD_ITEM_KEPT_FREE;
                ...
                try_placing_transport(world, item_idx, transport_count, placed_transport_count_ref, iteration, threshold_2, last_item);
                continue;
            }
            ...
            if ( blockade_count > *placed_blockade_count_ref && win_rand() % probablility < threshold )
            {
                if ( GetDistanceToCenter(x - 1, y) < iteration )  {
                    ...
                    {
                        world[item_idx] = WORLD_ITEM_BLOCK_EAST;
                        --puzzle_count_to_place;
                        *placed_puzzle_count_ref += 2;
                        world[item_idx-10] = WORLD_ITEM_KEPT_FREE;
                        world[item_idx+10] = WORLD_ITEM_KEPT_FREE;
                        world[item_idx+1] = WORLD_ITEM_PUZZLE_CANDIDATE;
                        world[item_idx-9] = WORLD_ITEM_KEPT_FREE;
                        world[item_idx+11] = WORLD_ITEM_KEPT_FREE;
                        ...
                        world[item_idx] = WORLD_ITEM_EMPTY;
            ...
            if (blockade_count > *placed_blockade_count_ref && win_rand() % probablility < threshold )
            {
                ...
                if (distance_is_less_than_iteration && (!item_above || item_above == WORLD_ITEM_KEPT_FREE) &&
                    (!item_below || item_below == WORLD_ITEM_KEPT_FREE) )
                {
                    world[item_idx] = WORLD_ITEM_BLOCK_WEST;
                    --puzzle_count_to_place;
                    *placed_puzzle_count_ref += 2;
                    world[item_idx-10] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+10] = WORLD_ITEM_KEPT_FREE;
            ...
            switch (item_above) {
                ...
                case WORLD_ITEM_BLOCK_SOUTH:
                    if ((item_before == WORLD_ITEM_NONE || item_before == WORLD_ITEM_KEPT_FREE)
                        ...
                        world[item_idx] = WORLD_ITEM_PUZZLE_CANDIDATE;
                        world[item_idx-1] = WORLD_ITEM_KEPT_FREE;
                        world[item_idx+1] = WORLD_ITEM_KEPT_FREE;
            ...
            if ( blockade_count > *placed_blockade_count_ref && win_rand() % probablility < threshold ) {
                ...
                if (distance_less_than_iteration && ( !item_before || item_before == WORLD_ITEM_KEPT_FREE )
                    ...
                    world[item_idx] = WORLD_ITEM_BLOCK_SOUTH;
                    --puzzle_count_to_place;
                    *placed_puzzle_count_ref += 2;
                    world[item_idx-1] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+1] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+10] = WORLD_ITEM_PUZZLE_CANDIDATE;
                    world[item_idx+9] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+11] = WORLD_ITEM_KEPT_FREE;
                    ++*placed_blockade_count_ref;
            ...
            if ( blockade_count > *placed_blockade_count_ref && win_rand() % probablility < threshold ) {
                ...
                if (distance_less_than_iteration && ( !item_before || item_before == WORLD_ITEM_KEPT_FREE )
                    && ( !item_after || item_after == WORLD_ITEM_KEPT_FREE ))
                {
                    world[item_idx] = WORLD_ITEM_BLOCK_NORTH;
                    --puzzle_count_to_place;
                    *placed_puzzle_count_ref += 2;
                    world[item_idx-1] = WORLD_ITEM_KEPT_FREE;
                    world[item_idx+1] = WORLD_ITEM_KEPT_FREE;
...
int Map::try_placing_transport(uint16_t *world, int item_idx, int transport_count, int *placed_transport_count_ref, int iteration, int threshold_2, int v36) {
    
    if (world[item_idx] == WORLD_ITEM_EMPTY && transport_count > *placed_transport_count_ref
        && ((win_rand()) & 7) < threshold_2 && v36 != WORLD_ITEM_TRAVEL_START && iteration > 2 )
    {
        world[item_idx] = WORLD_ITEM_TRAVEL_START;
...
#pragma mark -
int Map::AdditionalPuzzleLocations(int travels_to_place, uint16_t* world, int* placed_puzzles_count_ref){
    for(int i=0; i <= 400 && travels_to_place > 0; i++) {
        ...
        switch (item_neighbor) {
            ...
            case WORLD_ITEM_PUZZLE_CANDIDATE:
                ...
                if (!x_diff ) {
                    if (x > 0) world[world_idx - 1] = WORLD_ITEM_KEPT_FREE;
                    if (x < 9) world[world_idx + 1] = WORLD_ITEM_KEPT_FREE;
                } else if (!y_diff) {
                    if ( y > 0 ) world[world_idx - 10] = WORLD_ITEM_KEPT_FREE;
                    if ( y < 9 ) world[world_idx + 10] = WORLD_ITEM_KEPT_FREE;
                }
...
int Map::choose_puzzles_behind_blockades(uint16_t* world, uint16_t *puzzles) {
...
int Map::choose_additional_puzzles(uint16_t* world, uint16_t *puzzles, int placed_puzzles, int total_puzzle_count) {
    int i;
    int do_break = 0;
    for(i=0; i <= 200; i++) {
        int x,y;
...
int Map::map_count_stuff(uint16_t* world, int* travels, int* blockades, int *puzzles){
...
int Map::make_sure_last_puzzle_is_not_too_close_to_center(uint16_t *puzzles, int placed_puzzles) {
    int max_puzzle_x = 0, max_puzzle_y = 0;
    find_puzzle(puzzles, placed_puzzles-1, &max_puzzle_x, &max_puzzle_y);
    
    if(GetDistanceToCenter(max_puzzle_x, max_puzzle_y) < 3) {
        for(int y=0; y < 10; y++) {
            for(int x=0; x < 10; x++) {
                if(map_get(puzzles, x, y) >= 0
                   && GetDistanceToCenter(x, y) >= 3
                   && (x != max_puzzle_x || y != max_puzzle_y)) {
                    int replacement_puzzle = map_get(puzzles, x, y);
                    map_set(puzzles, max_puzzle_x, max_puzzle_y, replacement_puzzle);
                    map_set(puzzles, x, y, placed_puzzles-1);
...
int Map::find_puzzle(uint16_t* puzzles, int count, int *x, int *y){
    for(int max_puzzle_y = 0; max_puzzle_y < 10; max_puzzle_y++){
        for(int max_puzzle_x = 0; max_puzzle_x < 10; max_puzzle_x++){
            if(map_get(puzzles, max_puzzle_x, max_puzzle_y) == count) {
                *x = max_puzzle_x;
                *y = max_puzzle_y;
                return 0;
            }
...
int Map::place_intermediate_world_thing() {
    int placed_puzzles = 0;
    int travel_count = 0, puzzles_count = 0, blockades_count = 0;;
    map_count_stuff(tiles, &travel_count, &blockades_count, &puzzles_count);
    ...
    placed_puzzles = choose_puzzles_behind_blockades(tiles, (uint16*)puzzles);
    placed_puzzles = choose_puzzles_on_islands(tiles, (uint16*)puzzles, placed_puzzles);
    placed_puzzles = choose_additional_puzzles(tiles, (uint16*)puzzles, placed_puzzles, total_puzzle_count);
    
    make_sure_last_puzzle_is_not_too_close_to_center((uint16*)puzzles, placed_puzzles);
